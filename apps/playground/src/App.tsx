    ...
    <div className="card">
      ...
      <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
        {["GET", "POST", "PUT", "PATCH", "DELETE"].map((m) => (
          <button key={m} onClick={() => runMethod(m)} disabled={loading}>
            {m}
          </button>
        ))}
      </div>
...
function ConfigDemo() {
  ...
  const testAuth = async () => {
    ...
    try {
      ...
      if (err instanceof ApiError && err.status === 401) {
        setResult(
          "✅ Auth header sent (401 expected from httpbin if token invalid)",
        );
      } else if (err instanceof Error) {
  ...
  return (
    <div className="card">
      ...
      <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
        <button onClick={testTimeout} disabled={loading}>
          Test Timeout (2s)
        </button>
        <button onClick={testHeaders} disabled={loading}>
          Test Headers
        </button>
...
function AdvancedDemo() {
  ...
  return (
    <div className="card">
      ...
      <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
        <button onClick={testRetry} disabled={loading}>
          Test Retry (500)
        </button>
        <button onClick={testParamsSerializer} disabled={loading}>
          Params Serializer
        </button>
        <button onClick={testAbort} disabled={loading}>
          Abort Signal
        </button>
      </div>
      {logs.length > 0 && (
        <div
          style={{ textAlign: "left", fontSize: "0.8em", margin: "0.5rem 0" }}
        >
          {logs.map((l, i) => (
            <div key={i}>{l}</div>
          ))}
...
function TransformValidationDemo() {
  ...
  const testTransform = async () => {
    ...
    try {
      ...
      const data = await client.post<
        HttpBinPostResponse & { received: boolean },
        { original: boolean }
      >("/post", { original: true });
      setResult(data);
    } catch (err: unknown) {
  ...
  const testValidation = async (shouldFail: boolean) => {
    ...
    try {
      ...
      if (err instanceof Error)
        setResult(`❌ Validation Failed: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="card">
      ...
      <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
        <button onClick={testTransform} disabled={loading}>
          Test Transforms
        </button>
        <button onClick={() => testValidation(false)} disabled={loading}>
          Validate (Pass)
        </button>
        <button onClick={() => testValidation(true)} disabled={loading}>
          Validate (Fail)
        </button>
      </div>
      {result && (
...
function InterceptorDemo() {
  ...
  const runTest = async () => {
    ...
    const reqId = api.interceptors.request.use((config) => {
      setLogs((prev) => [
        ...prev,
        `⬆️ Request Interceptor: Adding custom header`,
      ]);
