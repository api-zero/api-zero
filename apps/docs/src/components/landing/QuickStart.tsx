          ...
          <div className="relative">
            <Reveal>
              ...
              <p className="mt-4 text-fd-muted-foreground">
                <code className="text-fd-primary">@api-zero/core</code> is all
                you need. React bindings and Zod contracts are separate packages
                you add only if your project already has React or Zod.
              </p>
            </Reveal>
              ...
              <Tabs items={MANAGERS.map((m) => m.name)}>
                {MANAGERS.map((manager) => (
                  <Tab key={manager.name} value={manager.name}>
                    <Code lang="bash" code={manager.command} />
                  </Tab>
                ))}
              </Tabs>
            </Reveal>
